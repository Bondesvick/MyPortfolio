using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace MyPortfolio.Models
{
    public class MyDetails
    {
        public string FirstName { get; set; }
        public string SecondName { get; set; }
        public string School1 { get; set; }
        public string School1Major { get; set; }
        public string School1Details { get; set; }
        public string School2 { get; set; }
        public string School2Major { get; set; }
        public string School2Details { get; set; }
        public string Skills { get; set; }
        public string Occupation { get; set; }
        public int YearsOfExperience { get; set; }
        public string Introduction1 { get; set; }
        public string Introduction2 { get; set; }
        public string Email { get; set; }
        public string ResumeLink { get; set; }
        public string Facebook { get; set; }
        public string Twitter { get; set; }
        public string Instagram { get; set; }
        public string LinkedIn { get; set; }
        public string GitHub { get; set; }
        public string ProfileImage1 { get; set; }
        public string ProfileImage2 { get; set; }
        public string Porfolio1 { get; set; }
        public string Porfolio2 { get; set; }
        public string Porfolio3 { get; set; }
        public string Porfolio4 { get; set; }
        public string GitRepos { get; set; }
        public string Porfolio1Image { get; set; }
        public string Porfolio2Image { get; set; }
        public string Porfolio3Image { get; set; }
        public string Porfolio4Image { get; set; }
    }
}